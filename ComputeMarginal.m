...
EvidenciaObservada = ObserveEvidence(Joint, E);

[M.var EOi VEi] = intersect(EvidenciaObservada.var, V);
    [dummy, mapM] = ismember(M.var, EvidenciaObservada.var);
    M.card = EvidenciaObservada.card(EOi);
    M.val = zeros(1,prod(M.card));

    for k = 1:length(EvidenciaObservada.val),
        RowObsEvidenceCond = IndexToAssignment(k, EvidenciaObservada.card);
        AM = RowObsEvidenceCond(mapM);
        idx = AssignmentToIndex(AM, M.card);
        M.val(idx) = M.val(idx) + EvidenciaObservada.val(k);
    end;    
    total = sum(M.val);
    M.val = M.val/total;
